...
class TestGithubRunPipeline:

    REQUIRED_INPUT_PARAMS = {
        'params': {
            'pipeline_owner': 'test-owner',
            'pipeline_repo_name': 'test-repo',
            'pipeline_workflow_file_name': 'workflow.yml'
        },
        'systems': {
            'github': {
                'password': 'test-token'
    ...
    def test_validation_fails_without_required_params(self, tmp_path, caplog):
        with pytest.raises(SystemExit) as exit_result:
            cmd = GithubRunPipeline(folder_path=str(tmp_path))
            cmd.run()

        assert exit_result.value.code == 1
        assert "Parameter 'params.pipeline_owner' is mandatory" in caplog.text
    ...
    def test_validation_succeeds_with_minimal_required_params(self, tmp_path):
        ...
        cmd = GithubRunPipeline(folder_path=str(tmp_path), input_params=input_params)

        assert cmd._validate() is True

    @patch('qubership_pipelines_common_library.v2.github.safe_github_client.SafeGithubClient.create_github_client')
    def test_execute_with_existing_pipeline_success(self, mock_create_client, tmp_path):
        input_params = copy.deepcopy(self.REQUIRED_INPUT_PARAMS)
        input_params['params']['use_existing_pipeline'] = 123

        mock_execution = Mock(spec=ExecutionInfo)
        mock_execution.get_status.return_value = ExecutionInfo.STATUS_SUCCESS
        mock_client = Mock()
        mock_create_client.return_value = mock_client
        mock_client.wait_workflow_run_execution.return_value = mock_execution
        ...
        mock_client.wait_workflow_run_execution.assert_called_once()
    ...
    def test_execute_async_mode_no_waiting(self, mock_create_client, tmp_path, caplog):
        ...
        mock_client = Mock()
        mock_create_client.return_value = mock_client
        mock_client.trigger_workflow.return_value = mock_execution
        ...
        mock_client.trigger_workflow.assert_called_once()
        mock_client.wait_workflow_run_execution.assert_not_called()
    ...
    def test_execute_with_custom_data_importer(self, mock_create_client, tmp_path):
        ...
        mock_create_client.return_value = mock_client
        mock_client.trigger_workflow.return_value = mock_execution
        mock_client.wait_workflow_run_execution.return_value = mock_execution_finished
    ...
    def test_save_execution_info_outputs_correct_params(self, tmp_path):
        ...
        mock_execution = Mock()
        mock_execution.get_url.return_value = "https://github.com/test/repo/actions/runs/123"
        ...
        cmd.context.output_param_set.assert_has_calls([
            call("params.build.url", "https://github.com/test/repo/actions/runs/123"),
